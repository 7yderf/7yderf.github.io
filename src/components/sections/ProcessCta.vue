<template>
  <section class="sc-section w-full px-4 py-4 lg:px-8">
    <div class="sc-article mx-auto w-full max-w-7xl flex-col items-center justify-between gap-8 rounded-3xl bg-gradient-to-r from-primary-soft via-bg-second to-accent-soft p-8 md:flex-row md:p-12">
      ...
      <div class="flex flex-nowrap items-start justify-center gap-3">
        <template v-for="(step, i) in steps" :key="step.label">
          ...
          >
            <Icon icon="mdi:arrow-right" class="text-ink-3" width="20" height="20" />
          </div>
        </template>
      </div>
    </div>
