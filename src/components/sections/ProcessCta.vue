<template>
  <section class="sc-section w-full px-4 py-12 lg:px-8 lg-2:py-24">
    <div class="sc-article mx-auto w-full max-w-7xl flex-wrap items-center justify-between gap-8 rounded-3xl bg-[linear-gradient(89.78deg,_#865BDA_0.19%,_#240F4E_115.48%)] p-6 lg-2:p-12">
      ...
      <div class="flex shrink-0 flex-nowrap items-start justify-center gap-3 m-auto">
        <template v-for="(step, i) in steps" :key="step.label">
          <div class="flex w-16 shrink-0 flex-col items-center gap-2 text-center md:w-20 lg-2:w-24">
            <img :src="step.icon" alt="" aria-hidden="true" class="h-14 w-14 object-contain md:h-20 md:w-20 lg-2:h-[7rem] lg-2:w-[7rem]">
            <span class="font-secondary text-xs font-semibold text-text-invert">{{ step.label }}</span>
          </div>
          <div
            v-if="i < steps.length - 1"
            class="hidden h-14 shrink-0 items-center sm:flex md:h-20 lg-2:h-[7rem]"
          >
            <img src="/icons/arrow.webp" alt="" aria-hidden="true" class="w-6 object-contain brightness-0 invert md:w-10 lg-2:w-[3rem]">
          </div>
        </template>
      </div>
    </div>
