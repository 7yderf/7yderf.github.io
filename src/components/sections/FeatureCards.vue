...
// Gradiente de fondo + color de botón + destino del CTA por card, en orden
// de aparición (Figma). El destino es la vista de campaña propia de cada
// solución; "Infraestructura soberana y nube híbrida" no tiene vista propia
// entre las ya construidas, así que su CTA cae en contacto.
const cardStyles = [
  { gradient: 'linear-gradient(180deg, #A9DCFB 0%, #FDFDFF 100%)', button: '#92CDF1', path: '/infraestructura-spei' },
  { gradient: 'linear-gradient(180deg, #DFCEFF 0%, #FDFDFF 100%)', button: '#D9C6FF', path: '/hardware-criptografico' },
  { gradient: 'linear-gradient(180deg, #FFCDAA 0%, #FFF2E8 100%)', button: '#FFB37C', path: '/contact' },
  { gradient: 'linear-gradient(180deg, #FFFAF7 0%, #FFFDF9 100%)', button: '#EEE0D8', path: '/cripto-as-a-service' },
]
...
/* Movil (<=768): el riel de grid quedaba desproporcionado -el font-size raiz
   TAMBIEN cae en este breakpoint (style.scss:96-98, 55%), asi que --slot-h
   en rem rendia bastante mas chico de lo que su valor sugiere, pero aun asi
   sobraba muchisimo hueco contra el alto real de la card (medido: renglon
   616px vs card ~280px). display:block apaga SOLO el mecanismo de fila alta
   -grid-template-rows/gap quedan inertes, no hace falta borrarlos, siguen
   sirviendo a la regla base de escritorio-. --stack-step sigue vivo: el
   padding-top escalonado + sticky ya solos sostienen una superposicion sutil
   sin el hueco, validado visualmente por Fredy. --slot-h no se declara aqui
   a proposito, quedaria sin uso. */
