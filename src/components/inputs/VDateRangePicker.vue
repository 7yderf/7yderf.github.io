<template>
  <div class="field field--picker" :data-Type="data">
    ...
    <div class="input input__box" :data-Type="data" :data-focus="isFocused" :data-error="isError" :data-empty="isEmpty">
      <FieldLabel
:label="label" :data="data" :data-focus="isFocused || !isEmpty" :form-name="formName"
        :data-height="height" :data-withIcon="!!icon" />
      <Field v-slot="{ field, meta, value }" v-model="fieldValue" :name="name">
        <DatePicker
v-bind="field" :id="`${label}-${formName}`" ref="datePicker" v-model="fieldValue" autocomplete="off"
          cancel-text="Cancelar" select-text="Aceptar" :readonly="readonly" :format="format" :value-type="valueType"
          :data-height="height" :locale="locale" :range="{ minRange: 1 }" @focus="onFocus()"
          @blur="onBlur(meta)" @update:model-value="handleUpdate" />
        <span class="input__bullet" style="cursor: pointer;" @click="listenerClick()">
...
onMounted(() => {
  if (props.defaultValue) {
    ...
    const parsedDate = props.defaultValue;
